...
Generate a 30-day launch sprint:

```powershell
python -m farm_loop.main --portfolio-once --portfolio-phase generate --dry-run --max-opportunities 10 --launch-sprint-output-dir out/launch-sprint
```
...
Generate the full ranked idea catalog:

```powershell
python -m farm_loop.main --portfolio-once --portfolio-phase generate --dry-run --max-opportunities 10 --all-ideas-output-dir out/all-ideas
```

The all-ideas catalog writes `all_revenue_ideas.json` and `ALL_REVENUE_IDEAS.md`. It ranks every discovered idea, groups ideas by revenue channel, shows the first asset to create, the monetization path, allowed distribution, and units needed for the configured milestones.
...
- `ALL_IDEAS_OUTPUT_DIR`, optional complete ranked idea catalog path such as `out/all-ideas`
